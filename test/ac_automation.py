...
class acmation:
    def __init__(self):
        self.root = ac_node("")  # 初始化根结点
        self.count = 0  # 模式串个数
        self.match = {}

    # 第一步：模式串建树
    def addkey(self, strkey, type):
        ...
        self.match[p.tail] = [strkey, type]

    def get_match_info(self, id):
        if self.match.has_key(id):
